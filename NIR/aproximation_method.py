...
for m in range(N):
    ...
    for n in range(N):
        ...
        if n == m:
            # электрическое поле
            E1y = k1**2*dx*1j/4*(1 + 1j*2/pi*(ln(gamma*k1*delta/2)-1))
            ...
            # электрчиеское поле
            E1y = k1*dx + k1*2*1j/pi * (-wmn*(atan(umn_plus/wmn) - atan(umn_minus/wmn))
            ...
            # магнитное поле 
            H1u = 1j/8*wmn*k1**2*dx + 1/(2*pi)*(atan(umn_plus/wmn) - atan(umn_minus/wmn)) # delta?
            H2u = 1j/8*wmn*k2**2*dx + 1/(2*pi)*(atan(umn_plus/wmn) - atan(umn_minus/wmn)) # delta?
            
            H1w = 1j/4*(H(0,k1*r_minus) - H(0,k1*r_plus))
            H2w = 1j/4*(H(0,k2*r_minus) - H(0,k2*r_plus))
        else:
            # электрчиеское поле
            E1y = k1**2*1j/4*dx*H(0,k1*rmn) # delta?
            E2y = k2**2*1j/4*dx*H(0,k2*rmn) # delta?
               
            # магнитное поле     
            H1u = 1j/4*k1*dx*(wmn/rmn*H(1,k1*rmn)) # delta?
            H2u = 1j/4*k2*dx*(wmn/rmn*H(1,k2*rmn)) # delta?
